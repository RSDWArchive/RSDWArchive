"""Compile USD_*.json (utility spell data) exports into SpellData.json."""
...
from compiledata import enrich_spell_entry, load_item_name_table, load_xp_event_tables_by_stem

INDENT = 2
FILE_GLOB = "USD_*.json"
SOURCE_DIR_ENV_VAR = "RSDW_SPELL_SOURCE_DIR"
...
def compact_spell_entry(raw: Any) -> dict[str, Any] | None:
    """Prefer UtilitySpellData export when present; otherwise first export."""
    if not isinstance(raw, list) or not raw or not isinstance(raw[0], dict):
        return None
    primary_exp = None
    picked_utility = False
    for exp in raw:
        if isinstance(exp, dict) and exp.get("Type") == "UtilitySpellData":
            primary_exp = exp
            picked_utility = True
            break
    ...
    if picked_utility and primary_exp is not raw[0]:
        fe = raw[0]
        out["primaryExportSource"] = "UtilitySpellData"
        if isinstance(fe, dict):
            out["skippedFirstExport"] = {"type": fe.get("Type"), "name": fe.get("Name")}
    else:
        out["primaryExportSource"] = "UtilitySpellData" if picked_utility else "firstExport"
    return out
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compile USD_*.json assets into SpellData.json.")
    parser.add_argument(
        "--output",
        default=str(Path(__file__).resolve().parent / "SpellData.json"),
    ...
    print("[DEBUG] CompileSpellData started", flush=True)
    ...
    for path in paths:
        ...
        try:
            ...
            continue
        compact = compact_spell_entry(raw)
        if compact is None:
    ...
    print(f"[INFO] Wrote SpellData -> {out_path}", flush=True)
    print(
