"""
Scan exported Unreal JSON for blueprint actor class paths.

Walks the dataset `json/` tree for files whose name starts with `BP_`, loads each
...
Writes `BPData.json` in this folder: an object keyed by blueprint type name with
relative path, parsed class path, package path, and `runtimePath` (Unreal mount
style: `/Game/...` for core `RSDragonwilds/Content/...`, or `/{FeatureName}/...`
for plugin game-feature content before `/Content/`).

Source root matches other tools: RSDW_BP_SOURCE_DIR, RSDW_JSON_ROOT, or highest
semver `*/json` under the repo.

Example:
  python CompileBPData.py
  python CompileBPData.py --json-root E:/RSDWArchive/0.11.1/json
"""
...
INDENT = 2
SOURCE_DIR_ENV_VAR = "RSDW_BP_SOURCE_DIR"
...
def blueprint_type_from_json_stem(stem: str) -> str:
    """e.g. BP_Foo -> BP_Foo_C; BP_Foo_C -> BP_Foo_C."""
    if stem.endswith("_C"):
        return stem
    return f"{stem}_C"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build BPData.json from BP_*.json exports.")
    parser.add_argument(
        "--json-root",
        type=Path,
        default=None,
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=_HERE / "BPData.json",
        help=f"Output JSON path (default: {_HERE / 'BPData.json'})",
    )
    ...
    for path in sorted(json_root.rglob("BP_*.json")):
        if not path.is_file():
            continue
        scanned += 1
        stem = path.stem
        if not stem.startswith("BP_"):
            continue
        expected_type = blueprint_type_from_json_stem(stem)
        try:
            raw = load_json(path)
    ...
    meta = {
        "generatedBy": "website/tools/BPData/CompileBPData.py",
        "jsonRoot": str(json_root),
        "counts": {
            "bpJsonFilesScanned": scanned,
            "blueprintsResolved": matched,
    ...
    output_payload = {"_meta": meta, "blueprints": ordered}
    ...
    print(
        f"[INFO] Wrote {out_path} ({matched} blueprints from {scanned} BP_*.json files)",
        flush=True,
    )
