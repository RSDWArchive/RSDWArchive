"""Compile DA_Consumable_Vestige_*.json exports into VestigeData.json."""
...
from compiledata import build_recipe_stem_index, enrich_vestige_entry, load_item_name_table

INDENT = 2
FILE_GLOB = "DA_Consumable_Vestige_*.json"
SOURCE_DIR_ENV_VAR = "RSDW_VESTIGE_SOURCE_DIR"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compile DA_Consumable_Vestige_*.json into VestigeData.json.")
    parser.add_argument(
        "--output",
        default=str(Path(__file__).resolve().parent / "VestigeData.json"),
    ...
    print("[DEBUG] CompileVestigeData started", flush=True)
    ...
    recipe_index = build_recipe_stem_index(source_root)
    for compact in entries.values():
        enrich_vestige_entry(compact, source_root, st_item_names, recipe_index)
    ...
    print(f"[INFO] Wrote VestigeData -> {out_path}", flush=True)
    print(
